...
// Fonction pour obtenir l'historique des commandes de l'utilisateur
function getUserOrderHistory() {
    ...
    $pdo = getDBConnection();
    $stmt = $pdo->prepare("SELECT id, date, status FROM orders WHERE user_id = :user_id ORDER BY date DESC");
    $stmt->execute(['user_id' => $_SESSION['user_id']]);
    $orders = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// Fonction pour obtenir les détails d'une commande pour l'utilisateur connecté
function getUserOrderDetails($orderId) {
    if (!isset($_SESSION['user_id'])) {
        echo json_encode(['error' => 'Utilisateur non connecté']);
        return;
    }
    ...
    $stmtOrder = $pdo->prepare("SELECT * FROM orders WHERE id = :orderId AND user_id = :user_id");
    $stmtOrder->execute(['orderId' => $orderId, 'user_id' => $_SESSION['user_id']]);
    $order = $stmtOrder->fetch(PDO::FETCH_ASSOC);
...
// Fonction pour repasser une commande identique
function reorder($orderId) {
    ...
    // Récupérer les articles de la commande
    $stmtItems = $pdo->prepare("SELECT * FROM order_items WHERE order_id = :orderId");
    $stmtItems->execute(['orderId' => $orderId]);
    $items = $stmtItems->fetchAll(PDO::FETCH_ASSOC);

    if (empty($items)) {
        echo json_encode(['error' => 'Aucun article trouvé pour cette commande']);
        return;
    }
    ...
    try {
        ...
        foreach ($items as $item) {
            $stmt = $pdo->prepare("SELECT stock FROM products WHERE id = :productId");
            $stmt->execute(['productId' => $item['product_id']]);
            $product = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        echo json_encode(['success' => 'Commande renouvelée avec succès']);
    } catch (Exception $e) {
        // Annuler la transaction en cas d'erreur
        $pdo->rollBack();
        echo json_encode(['error' => 'Erreur lors du renouvellement de la commande : ' . $e->getMessage()]);
    }
}

// Fonction pour transférer les produits de la commande dans le panier
function addOrderToCart($orderId) {
    ...
    // Vérifier le stock pour chaque produit et ajouter au panier
    foreach ($items as $item) {
        $stmt = $pdo->prepare("SELECT * FROM products WHERE id = :productId");
        $stmt->execute(['productId' => $item['product_id']]);
        $product = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($item['quantity'] > $product['stock']) {
            echo json_encode(['error' => 'Stock insuffisant pour le produit ID ' . $item['product_id']]);
            return;
        }
        ...
        foreach ($_SESSION['cart'] as &$existingItem) {
            if ($existingItem['id'] == $cartItem['id'] && $existingItem['options'] == $cartItem['options']) {
                $newQuantity = $existingItem['quantity'] + $cartItem['quantity'];
                if ($newQuantity > $product['stock']) {
                    echo json_encode(['error' => 'Quantité totale demandée supérieure au stock disponible pour le produit ID ' . $product['id']]);
                    return;
                }
    ...
    echo json_encode(['success' => 'Produits ajoutés au panier']);
}
